                                          ...
                                          size_t offset_bytes)
{
    ...
    VerifyOrExit(keyEntryStorage != NULL,
            ChipLogError(DeviceLayer, "%s new KeyEntryStorage", __func__);
            err = CHIP_ERROR_NO_MEMORY;
            );

    SuccessOrExit(err = keyEntryStorage->FindKeyNvramID(nvramID, key));

    byte_count = wiced_hal_read_nvram(nvramID,
            value_size, (uint8_t *)value, &result);
    VerifyOrExit(result == WICED_SUCCESS,
            ChipLogError(DeviceLayer, "%s wiced_hal_read_nvram %u", __func__, result);
...
CHIP_ERROR KeyValueStoreManagerImpl::_Put(const char * key, const void * value, size_t value_size)
{
    ...
    VerifyOrExit(err == CHIP_NO_ERROR,
            ChipLogError(DeviceLayer, "%s AllocateEntry %s", __func__, ErrorStr(err)));

    byte_count = wiced_hal_write_nvram(nvramID,
            value_size, (uint8_t *)value, &result);
...
{
    wiced_result_t result;
    const uint16_t byte_count = wiced_hal_read_nvram(PLATFORM_NVRAM_ID_ENTRY_INFO_STRING,
            sizeof(mKeyEntries), (uint8_t *)mKeyEntries, &result);
...
KeyValueStoreManagerImpl::KeyEntryStorage::~KeyEntryStorage(void)
{
    ...
    {
        wiced_result_t result;
        const uint16_t byte_count = wiced_hal_write_nvram(PLATFORM_NVRAM_ID_ENTRY_INFO_STRING,
                sizeof(mKeyEntries), (uint8_t *)mKeyEntries, &result);
        if (byte_count != sizeof(mKeyEntries) || result != WICED_SUCCESS)
        {
            ChipLogError(DeviceLayer, "%s wiced_hal_write_nvram %u", __func__, result);
...
{
    ReturnErrorCodeIf(keyLength == 0 || keyLength > sizeof(mKeyEntries[0].mKey),
